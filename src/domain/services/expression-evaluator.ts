...
export type EvaluationResult = { ok: true; value: number } | { ok: false };
  ...
  while (i < expression.length) {
    ...
    if (OPERATOR_CHARS.has(char)) {
      ...
      if (expectsOperand) {
        ...
        while (
          j < expression.length &&
          NUMBER_CHAR_PATTERN.test(expression[j])
        ) {
...
function reduceByOperators(
  tokens: Token[],
  operators: Operator[],
): Token[] | null {
  if (tokens.length === 0 || tokens[0].kind !== 'number') {
    return null;
  }
...
function evaluateTokens(tokens: Token[]): number | null {
  ...
  if (!afterMultiplyDivide) {
    return null;
  }

  const afterAddSubtract = reduceByOperators(afterMultiplyDivide, ['+', '-']);
  if (!afterAddSubtract || afterAddSubtract.length !== 1) {
    return null;
  }
