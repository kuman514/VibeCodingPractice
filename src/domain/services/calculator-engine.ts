...
import { evaluateArithmeticExpression, formatEvaluationValue } from "@/domain/services/expression-evaluator";
...
export function inputOperator(state: CalculatorState, operator: Operator): CalculatorState {
  ...
  expression = isOperatorChar(lastChar) ? expression.slice(0, -1) + operator : expression + operator;
...
export function deleteLastCharacter(state: CalculatorState): CalculatorState {
  if (state.justEvaluated) return createInitialCalculatorState();
...
export function evaluateCalculatorExpression(state: CalculatorState): CalculatorState {
  if (state.expression === "") return state;

  const evaluation = evaluateArithmeticExpression(state.expression);
  const resultText = evaluation.ok ? formatEvaluationValue(evaluation.value) : "Error";
...
export function getCalculatorDisplayLines(state: CalculatorState): CalculatorDisplayLines {
  ...
  const bigLine = state.justEvaluated ? state.result ?? "0" : segment && segment !== "-" ? segment : "0";
