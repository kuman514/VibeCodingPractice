...
import { Operator, createInitialCalculatorState } from "@/domain/models/calculator";
import {
  clearCalculator,
  deleteLastCharacter,
...
import { EqualsEffects, generateEqualsEffects } from "@/adapters/ui/calculator/fx-generator";
...
export function CalculatorApp() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const key = event.key;
      if (/^[0-9]$/.test(key)) {
        setState((prev) => inputDigit(prev, key));
      } else if (key === ".") {
        setState((prev) => inputDecimalPoint(prev));
      } else if (key === "+" || key === "-" || key === "*" || key === "/") {
        setState((prev) => inputOperator(prev, key as Operator));
    ...
    <div style={rootStyle}>
      ...
      <div
        style={{
          ...
          borderRadius: "6cqw",
          boxShadow: "max(8px,2.4cqw) max(8px,2.4cqw) 0 rgba(0,0,0,.6), 0 0 9cqw rgba(163,230,53,.28)",
        }}
      >
        <div
          style={{
        ...
        >
          ...
          <span style={{ display: "flex", gap: "1.4cqw" }}>
            <span style={{ width: "2.6cqw", height: "2.6cqw", borderRadius: "50%", background: "#ff3b6b", boxShadow: "0 0 2cqw #ff3b6b" }} />
            <span style={{ width: "2.6cqw", height: "2.6cqw", borderRadius: "50%", background: "#ffd23f", boxShadow: "0 0 2cqw #ffd23f" }} />
            <span
              style={{
                width: "2.6cqw",
                height: "2.6cqw",
        ...
        <CalculatorKeypad
          ...
          onOperator={(operator) => setState((prev) => inputOperator(prev, operator))}
